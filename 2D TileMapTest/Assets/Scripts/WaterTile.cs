...
public class WaterTile : Tile {
	...
	public override void RefreshTile(Vector3Int position, ITilemap tilemap) {
		//Runs through all the tile's neighbours
		for (int y = -1; y <= 1; y++)  {
			for (int x = -1; x <= 1; x++) { 
				//We store the position of the neighbour 
				Vector3Int nPos = new Vector3Int(position.x + x, position.y + y, position.z);

				//If the neighbour has water on it
				if ( HasWater(tilemap, nPos)){
					//Them we make sure to refresh the neighbour aswell
	...
	public override void GetTileData(Vector3Int location, ITilemap tilemap, ref TileData tileData) {
		//Makes an empty string as compostion, neededto change the sprite
		string composition = string.Empty;
		//Runs through all neighbours 
		for (int x = -1; x <= 1; x++) { 
			for (int y = -1; y <= 1; y++) {
				//Makes sure that we aren't checking our self
				if (x != 0 || y != 0) {
